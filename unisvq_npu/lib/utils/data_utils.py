...
def sample_rp1t(tokenizer, size=128, ctx_size=2048, nproc=1):
    dataset = load_dataset('YOUR_DATASET_PATH/RedPajama-Data-1T-Sample',split='train')
...
def load_quip(save_name, cb, args, device, return_scales=False):
    ...
    try:
        ...
        _map_dev = _get_device()
    except ImportError:
        _map_dev = torch.device('cuda', device)
    dict_loaded = torch.load(save_name,
                             map_location=_map_dev)
    ...
    for i in range(len(devset) // batch_size):
        logits.append(
            model(devset[i * batch_size:(i + 1) *
                         batch_size].to(device=next(model.parameters()).device))['logits'].cpu())
