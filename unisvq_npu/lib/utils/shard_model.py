...
# from . import graph_wrapper  # CUDA-only, not available on NPU
...
class ShardTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        try:
            ...
            FusedQuantizedLinear = None
        from lib.linear.quantized_linear import QuantizedLinear
        for name, module in self.shards.named_modules():
            if isinstance(module, QuantizedLinear) or (FusedQuantizedLinear is not None and isinstance(
