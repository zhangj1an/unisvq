...
class QuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        self.has_bias = bias
        if self.has_bias:
            self.register_buffer('bias', torch.ones(out_features, dtype=torch.float16))
        ...
        self.codebook_class = codebook.get_quantized_class(self.codebook_id.item())(self.Qidxs.device, codebook_bit=codebook_bit)
    ...
    def no_ckpt_forward(self, input):
        if not self.built_codebook_class:
            ...
            self.SV.data = self.SV.data * self.Wscale
