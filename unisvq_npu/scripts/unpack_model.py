...
try:
    ...
    model = model.to(device="npu:0") if torch.npu.is_available() else model.to(device="cuda")
...
try:
    import torch_npu  # noqa: F401
    _dev = torch.device("npu:0") if torch.npu.is_available() else torch.device("cuda:0")
except (ImportError, AttributeError):
    _dev = torch.device("cuda:0")
over_write_model.to_empty(device=_dev)
