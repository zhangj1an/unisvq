...
def incoherence_preprocess(H, W, args):
    ...
    while retry <= 3:
        try:
            ...
            if torch.all(torch.isfinite(Lhr)):
                break
            else:
                glog.warning(f"LHr is not finite; scaling sigma reg: {args.sigma_reg} -> {(10**(retry+1))*args.sigma_reg}")
        except:
            retry += 1
...
def quantize(H_orig, W_orig, rank, codebook_orig, args, device='cpu'):
    ...
    # block LDL
    block_LDL_out = utils.block_LDL(Hr, codebook.codesz, sigma_reg=args.sigma_reg)
    ...
    for wscale_grid_search_factor in [1.0]: # torch.linspace(0.8, 1.2, args.scale_search_iters):
        ...
        try:
            ...
            print(hatW_search.dtype, W_orig.dtype, H_orig.dtype)
            raise RuntimeError
    ...
    attr = {
        ...
        'SV': (SV * best_Wscale.to(SV.device)).to(torch.float16).to(orig_device),  # fuse Wscale into SV
        'scaleWH': scaleWH,
    }
...
def quantize_linear(weights, save_path, hessian_path, cb, args, device='cpu', bias=None, save_result=True):
    ...
    # print("after quant", attr["Qidxs"].max(), attr["Qidxs"].min())
    if len(scales) == 1:
        attr['SV'] *= scales[0]
        scales = [1.0]
    attr.update({
